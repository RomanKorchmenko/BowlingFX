    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_268.rg, _Normals))).rgb;
...
////// Lighting:
                ...
                float3 finalColor = (attenuation*_LightColor0.rgb*((tex2D(_Diffuse,TRANSFORM_TEX(node_268.rg, _Diffuse)).rgb*_Color.rgb*(round((max(0,dot(lightDirection,node_41))*node_216))/node_216))+UNITY_LIGHTMODEL_AMBIENT.rgb+(round((node_216*pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0)))))/node_216)));
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_269.rg, _Normals))).rgb;
                float3 normalDirection = normalize( mul( normalLocal, tangentTransform ) );
...
////// Lighting:
                ...
                float3 node_41 = normalDirection;
                float node_216 = _Bands;
                float3 finalColor = (attenuation*_LightColor0.rgb*((tex2D(_Diffuse,TRANSFORM_TEX(node_269.rg, _Diffuse)).rgb*_Color.rgb*(round((max(0,dot(lightDirection,node_41))*node_216))/node_216))+UNITY_LIGHTMODEL_AMBIENT.rgb+(round((node_216*pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0)))))/node_216)));
/// Final Color:
