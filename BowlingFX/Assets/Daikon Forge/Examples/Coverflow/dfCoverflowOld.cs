...
public class dfCoverflowOld : MonoBehaviour
{
	...
	void ItemResize()
	{
//		if(Screen.width>Screen.height)//landskape
//		itemSize = Screen.height - 20;
//		else itemSize = Screen.width - 20;

		if(Screen.height>Screen.width)//portrait
		{
			
			if(tutor)itemSize = Mathf.FloorToInt(1.0f*Screen.width - Screen.width/offcet);
			else
				itemSize = Mathf.FloorToInt(Screen.width - Screen.width/offcet);
		}
		if(Screen.height<Screen.width)//landskape
		{
			if(tutor)itemSize = Mathf.FloorToInt(1.0f*Screen.height - Screen.height/offcet);
			else {
				itemSize = Mathf.FloorToInt(Screen.height - Screen.height/offcet);
			}
		}
